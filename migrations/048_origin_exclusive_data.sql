-- 048_origin_exclusive_data.sql — "nobody should have any data on any
-- character other than their origin" (Tyson, 2026-07-17).
--
-- Reverses two earlier decisions, by owner directive:
--   * 046's "non-origin lines keep progress, they just cannot render"
--   * 047's reforge rule "the old origin stays collected"
-- From now on, binding (first or reforge) is EXCLUSIVE: every user_paths
-- row that is not the origin is deleted, and every champion-bond row that
-- is not the origin's default champion is deleted. Premium overlays
-- (gymerica) and PURCHASES (skins, palettes, character unlocks) are never
-- touched — purchases are not progression data.
-- firstbound_origin is untouched: permanent history, not progression.
--
-- Applied together with a one-off cleanup for existing origin-havers
-- (executed 2026-07-17 via the management API, not part of this file):
--   delete from user_paths up using profile p
--     where p.user_id = up.user_id and p.origin_path is not null
--       and up.path <> p.origin_path;
--   (plus the matching user_champion_bond delete; no rows matched).
