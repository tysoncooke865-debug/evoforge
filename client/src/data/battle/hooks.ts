...
export interface BattleRound {
  ...
  round_no: number;
  kind: string;
  spec: { objectKey?: string; targetEffectiveKg?: number; displayKg?: number };
...
export function useBattleBundle(matchId: string | null) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<BattleBundle> => {
      const [m, p, r, e, s] = await Promise.all([
        ...
        supabase.from('battle_rounds').select('*').eq('match_id', matchId!).eq('round_no', 1).limit(1),
        supabase
          .from('battle_events')
      ...
      const firstError = m.error ?? p.error ?? r.error ?? e.error ?? s.error;
      if (firstError) throw firstError;
      return {
        match: ((m.data ?? [])[0] ?? null) as BattleMatch | null,
        participants: (p.data ?? []) as BattleParticipant[],
        round: ((r.data ?? [])[0] ?? null) as BattleRound | null,
