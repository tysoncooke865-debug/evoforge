/**
 * PAGE LAB VARIANT — "COMPACT": the workout-logger UI redesign, forked from
 * the baseline (which is itself the diff-zero fork of workout.tsx). On top
 * of the baseline's lab edits (named export, lab back(), mock-safe
 * finish/reopen/activation shims) this variant:
 *   - deletes the rest timer entirely (user decision — the rest store is
 *     module-global and shared with the live app);
 *   - dedupes the header: the kicker is the ONE set-progress text; the
 *     progress GlowCard becomes a thin ProgressStrip;
 *   - collapses the plan-source text + reorder toggle into one compact row
 *     ({SOURCE} · {DAY} — EDIT ORDER);
 *   - owns per-exercise collapse state (absent = expanded, never
 *     auto-collapsed) and renders the redesigned CompactExerciseCard.
 * Data-model limits, accepted: no rest-time field exists anywhere, and
...
import { CompactExerciseCard } from '@/lab/variants/workout/compact/exercise-card';
 ...
 */
export function WorkoutCompact() {
  ...
  // COMPACT (req 4/15): per-exercise collapse, page-owned so it survives the
  // reorder-mode list swap. Keyed by NAME; absent = expanded; never
  // auto-collapsed. Reset with everything else in the keyRef effect below.
  ...
  return (
    ...
    <ScreenShell>
      ...
      {/* THE ONE PROGRESS DISPLAY (req 2): the header kicker owns the N/M
          text; this strip is just the thin bar + the XP earned. No capsule,
          no duplicate counters. Glow on the fill is policy-sanctioned. */}
      <View className="flex-row items-center gap-s3">
        <View className="flex-1 overflow-hidden rounded-pill bg-surface-3" style={{ height: 6 }}>
          <View
            style={{
              width: `${dayPct}%`,
              height: '100%',
      ...
      {/* PLAN ROW (req 3): source · day on the left (the borrowed-source
          honesty survives, warn-coloured), EDIT ORDER on the right — one
          compact row replaces the old FROM-text + full-width reorder panel. */}
      {!isAdhoc && (resolved.from !== null || resolved.routine !== undefined) ? (
        <View className="flex-row items-center justify-between" style={{ minHeight: 32 }}>
          <Text
            className="flex-1 pr-s2"
          ...
          >
            {resolved.from === null
              ...
              : borrowedFrom !== null
                ...
                : `${SOURCE_LABEL[preferredSource]} · ${workoutName.split(' - ')[0].toUpperCase()}`}
          </Text>
          {editable && plan.length > 1 ? (
            <Pressable
              ...
              className="items-center justify-center pl-s2"
              style={{ minHeight: 44 }}
            >
              <Text
                allowFontScaling={false}
                style={{ fontSize: 10, letterSpacing: 1, color: colors.accent, ...pixelFont() }}
              >
      ...
      {/* REORDER MODE: the EDIT ORDER control lives on the plan row above;
          while active, the compact list stands in for the logging cards and
          the order persists in today's session overrides — unchanged. */}
      {reordering && editable ? (
        <ReorderableList
          items={plan}
          keyOf={(e) => e.exercise}
      ...
      {reordering ? null : plan.map((entry, i) => {
        ...
        return (
          <CompactExerciseCard
            ...
            // COMPACT: page-owned collapse (absent = expanded, req 4/15) and
            // the honest remove semantics for the ⋯ menu (req 17).
            collapsed={!!collapsedCards[exercise]}
            onToggleCollapsed={() =>
              setCollapsedCards((prev) => ({ ...prev, [exercise]: !prev[exercise] }))
            }
            removeDegradesToSkip={removeAction(facts) !== 'remove'}
          />
        );
      })}
    ...
    {/* COMPACT: no floating rest timer — excluded by design (req 25). */}
    </View>
  );
}
