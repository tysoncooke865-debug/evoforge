/**
 * PAGE LAB — CALCULATOR copy of src/ui/fuel/fuel-hero.tsx (fork recipe:
 * copied because it changes; re-sync when the live file moves). The copy's
 * ONE addition: an optional `rates` prop — when set, the CUT and BULK boxes
 * in the goal switcher carry a small caption naming the rate that priced
 * them (−loss / +gain kg/wk); the asymmetry has to be visible or it reads
 * as a rounding bug. Every testID and all behaviour are the live file's;
 * FuelMasthead rides along unchanged.
 */
/**
...
export function FuelHero({
  ...
  /** CALCULATOR variant: the dual rates behind the triple — CUT and BULK
   *  caption theirs; null = unknowable (manual target). */
  rates?: { loss: number; gain: number } | null;
  ...
  const goalSwitcher = (
    <View className="flex-row" style={{ gap: 8, opacity: goalBusy ? 0.5 : 1 }}>
      {GOALS.map((g) => {
        const active = g === goal;
        // CALCULATOR variant: MAINTAIN has no rate; the other two name theirs.
        const rateCaption =
          rates === null
            ? null
            : g === 'lose'
