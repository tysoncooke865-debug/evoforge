/**
 * PAGE LAB — COUNTERWEIGHT's fork of ui/fuel/fuel-hero.tsx (copied beside
 * the variant per the README rule: never edit src/ui/ for a variant's sake).
 * Diverges from the live hero in three ways, the batch's thesis:
 *   1. THE GOAL SWITCHER IS GONE. Nobody toggles from losing 1 kg/wk to
 *      gaining 1 kg/wk between meals — the three-chip switcher is replaced
 *      by two DISPLAY boxes: the goal in force on the LEFT (headed CUT or
 *      BULK, with its rate), MAINTAIN on the RIGHT for reference. A
 *      maintain goal collapses to ONE full-width box. Changing goal is a
 *      decision made inside RECALCULATE.
 *   2. EDIT → SET MANUALLY (same pencil, honest name — it opens a plain
 *      kcal popup, not an editor).
 *   3. The "since {date}" provenance line joins its row's pixel font
 *      instead of rendering in the system sans.
 * Everything else is verbatim from the live hero.
 */
...
export function FuelHero({
  ...
  // THE GOAL BOXES (fork divergence #1): display, not controls. The goal in
  // force reads on the LEFT with its stored kcal and rate; MAINTAIN sits on
  // the RIGHT for reference — or one full-width box when the goal IS
  // maintain. Changing either number goes through ✦ RECALCULATE.
  const goalBox = (
    <View
  ...
  const targetActions = (
    <View className="flex-row items-center">
      ...
      {/* Fork divergence #3: the provenance line joins its row's pixel type
          instead of standing out in the system sans. */}
      <Text
 ...
 * NO-TARGET-YET fallback in the variant (a brand-new athlete with nothing to
