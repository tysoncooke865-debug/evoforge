/**
 * PAGE LAB — COUNTERWEIGHT: the RECALCULATE sheet. The live ✦ RECALCULATE
 * opens the AI intake, which short-circuits to a read-only review —
 * effectively a dead end (HANDOVER, the 2026-08-21 finding). This sheet is
 * the replacement: a LOCAL, fully editable calculator. Every number comes
 * from domain maths on the device (dualRateTargets over Mifflin–St Jeor);
 * no network, no AI.
 *
 * Adapted from the culled CALCULATOR prototype
 * (`git show d120681:client/src/lab/variants/fuel/calculator/recalculate-sheet.tsx`)
 * for the batch's two-box thesis: the GOAL is picked as a chip (section 3),
 * ONE rate row edits the chosen goal's rate (hidden for maintain — the
 * other rate is carried, both persist), and the preview mirrors the hero's
 * goal + maintain boxes instead of a three-way selector.
 *
 * Structure is the ManualTargetSheet's bottom-sheet Modal, scrollable
 * because five sections outgrow one screen. APPLY writes through the lab
 * save-target shim — same mutation shape as the live path, built and pinned
 * in ./model (buildSavePayload).
 */
...
/** The rate chips — [0, MAX_RATE_KG_PER_WEEK] in plate steps. */
