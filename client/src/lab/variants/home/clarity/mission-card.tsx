      ...
      {showRewards || evoPerSession !== null || evoLabel ? (
        <View className="mt-s2" testID="mission-rewards">
          <Text
            ...
            allowFontScaling={false}
            style={{ fontSize: 10, letterSpacing: 1.8, ...pixelFont(false) }}
          >
            ESTIMATED REWARDS
          </Text>
          ...
          {evoLabel ? (
            <Text
              ...
              numberOfLines={1}
              allowFontScaling={false}
              style={{ fontSize: 10, letterSpacing: 1.2, color: colors['text-dim'], ...pixelFont(false) }}
              testID="mission-evo"
            >
              {'PRIMARY BENEFIT  '}
...
function Kicker({ children }: { children: React.ReactNode }) {
  return (
    <Text className="text-2xs font-bold text-text-dim" numberOfLines={1} ellipsizeMode="tail" style={{ letterSpacing: 2 }}>
      {children}
    </Text>
  );
}
