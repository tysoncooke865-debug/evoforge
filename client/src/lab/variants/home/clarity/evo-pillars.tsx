/**
 * PAGE LAB — CLARITY copy of src/ui/home/evo-pillars.tsx (fork recipe:
 * copied because it changes; re-sync when the live file moves). The copy's
 * constraints: pillar labels hold the 10px floor on text-dim, and the
 * weakest pillar is named FOCUS — the label must say what the warn fill
 * means, and "edge" said the opposite of what the colour marks.
 */
        ...
        {HORIZONS.map((h) => {
          ...
          return (
            ...
            >
              <Text
                allowFontScaling={false}
                style={{ fontSize: 10, color: active ? colors.epic : colors['text-dim'], ...pixelFont() }}
              >
                {h}W
              </Text>
            </Pressable>
  ...
  return (
    <View className="mt-s2">
      <View className="flex-row items-baseline justify-between">
        ...
        >
          {label}
          {/* FOCUS, because the warn fill means "this is where the work
              goes" — the label must say what the colour means. */}
          {weakest ? <Text style={{ color: colors.warn }}>{'  ·  FOCUS'}</Text> : null}
        </Text>
        <Text
          allowFontScaling={false}
        ...
        >
          {value}
          {gain > 0 ? (
            <Text style={{ fontSize: 10, color: colors.epic }}>{`  →${value + gain}`}</Text>
          ) : null}
        </Text>
      </View>
      <View
