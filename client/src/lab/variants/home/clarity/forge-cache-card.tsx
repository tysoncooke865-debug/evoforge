/**
 * PAGE LAB — CLARITY copy of src/ui/home/forge-cache-card.tsx (fork
 * recipe: copied because it changes; re-sync when the live file moves).
 * The copy's constraint: this card sits BELOW the MissionCard in the
 * clarity entry, so its ghost door reads VIEW TRAIN PLAN — the mission
 * label belongs to exactly one card. The anti-dark-pattern copy
 * ("PROGRESS SAVED · rest days count · nothing expires") is verbatim.
 */
        ...
        {needsRest ? (
          ...
          <View style={{ flex: 1 }}>
            ...
            <Link href={'/train' as never} asChild>
              {/* VIEW TRAIN PLAN, never a second TODAY'S MISSION: the
                  MissionCard directly above already owns that label and its
                  door — one page, one mission door. */}
              <NeonButton
                title="VIEW TRAIN PLAN"
                variant="ghost"
                pixel
