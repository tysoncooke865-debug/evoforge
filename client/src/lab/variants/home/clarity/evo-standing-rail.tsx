/**
 * PAGE LAB — CLARITY copy of src/ui/home/evo-standing-rail.tsx (fork
 * recipe: copied because it changes; re-sync when the live file moves).
 * The copy's constraints: the rail rests at full opacity — 10px type
 * cannot afford a dimming baseline — and its resting tint is text-dim,
 * the AA tier. The gain's breath and the empty-over-vague rule survive.
 */
...
export function EvoStandingRail({ width }: { width?: number }) {
  ...
  // The rail rests FULLY LIT: a resting baseline under 1 would hold 10px
  // type below the AA floor for every athlete who isn't climbing. The
  // breath is the gain's reward only.
  const climbing = change !== null && change > 0;
  const style = useAnimatedStyle(() => ({
    opacity: reduced || !climbing ? 1 : 0.7 + glow.value * 0.3,
  }));

  if (line === null) return null;
  const tint = climbing ? colors.success : standing?.position === 1 ? colors.legendary : colors['text-dim'];
  ...
  return (
    <Animated.View style={[{ marginTop: 6, width, alignItems: 'center' }, style]}>
      ...
      {standing && standing.chasingRating !== null && standing.chasingName ? (
        <Text
          className="mt-s1 text-2xs text-text-dim"
          numberOfLines={1}
