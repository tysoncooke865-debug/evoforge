/**
 * PAGE LAB — CLARITY copy of src/ui/home/home-header.tsx (fork recipe:
 * copied because it changes; re-sync when the live file moves). The copy's
 * constraint: the level module's labels never render under 9px (the one
 * plate where 10 doesn't fit over the 88pt bar) and sub-12px type sits on
 * text-dim, the AA tier.
 */
    ...
    <View className="w-full flex-row items-start justify-between" style={{ gap: 10, zIndex: 10 }}>
      ...
      >
        ...
        >
          ...
          {/* 9px, the module's hard fit: the label must not outgrow the
              88pt bar below it — everywhere without that plate the floor is
              10. Dim, not mute: sub-12px labels take the AA tier. */}
          <Text
            className="text-text-dim"
            allowFontScaling={false}
            style={{ fontSize: 9, letterSpacing: 1.2, ...pixelFont(false) }}
          >
            FORGE LEVEL
          </Text>
