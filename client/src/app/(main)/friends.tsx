...
export default function FriendsScreen() {
  ...
  return (
    <ScreenShell>
      ...
      <GlowCard>
        <Text allowFontScaling={false} style={{ fontSize: 10, color: tokens.colors.accent, letterSpacing: 1.5, ...pixelFont(false) }}>
          YOUR ADD CODE
        </Text>
        <Text selectable allowFontScaling={false} style={{ marginTop: 4, fontSize: 34, color: tokens.colors.text, letterSpacing: 8, ...pixelFont() }} testID="my-friend-code">
          {code.data ?? '······'}
        </Text>
        ...
        <TextInput
          ...
          style={{ letterSpacing: 8, borderColor: entry.trim().length === 6 ? `${tokens.colors.accent}8c` : tokens.colors.border }}
          placeholder="——————"
      ...
      {requests.data && requests.data.length > 0 ? (
        <View className="gap-s2">
          ...
          {requests.data.map((r) => (
            <View key={r.id} className="flex-row items-center justify-between rounded-xl border p-s3" style={{ borderColor: `${tokens.colors.epic}59`, backgroundColor: 'rgba(168,85,247,0.06)' }}>
              ...
              <View className="flex-row items-center gap-s2">
                ...
                <Pressable onPress={() => respond.mutate({ id: r.id, accept: true })} accessibilityRole="button" testID={`invite-accept-${r.id}`} className="rounded-lg px-s3" style={{ minHeight: 40, justifyContent: 'center', backgroundColor: tokens.colors.accent }}>
      ...
      {friends.data && friends.data.length > 0 ? (
        friends.data.map((f) => {
          ...
          const col = lead === 'up' ? tokens.colors.success : lead === 'down' ? tokens.colors.danger : tokens.colors['text-dim'];
          return (
