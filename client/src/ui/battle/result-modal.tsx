...
export function BattleResultModal({
  ...
  const accent = versus ? (won ? colors.accent : colors.danger) : won ? colors.success : colors.danger;
  ...
  return (
    <Modal visible={visible} transparent animationType="fade">
      <View style={{ flex: 1, backgroundColor: 'rgba(2,6,14,0.86)', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
        <View className="w-full rounded-xl border p-s5" style={{ maxWidth: 380, borderColor: `${accent}66`, backgroundColor: colors.surface, shadowColor: accent, shadowOpacity: 0.4, shadowRadius: 24 }}>
          <Text style={{ fontSize: 34, textAlign: 'center', color: accent, textShadowColor: `${accent}88`, textShadowRadius: 18, ...pixelFont() }}>
            {heading}
          </Text>
          <Text style={{ textAlign: 'center', marginTop: 2, fontSize: 10, color: colors['text-mute'], fontFamily: PIXEL, letterSpacing: 1 }}>
            {subheading}
          </Text>
          ...
          {!won && !versus ? (
            <View className="mt-s3 rounded-lg border p-s3" style={{ borderColor: `${colors.accent}45`, backgroundColor: 'rgba(34,211,238,0.06)' }}>
              <Text style={{ fontSize: 9, color: colors.accent, fontFamily: PIXEL, letterSpacing: 1 }}>TACTICAL TIP</Text>
              <Text style={{ marginTop: 3, fontSize: 13, color: colors.text, lineHeight: 18 }}>{tip}</Text>
            </View>
          ) : null}
...
function SummaryRow({ label, value }: { label: string; value: string }) {
  ...
  return (
    <View className="flex-row items-center justify-between">
      <Text style={{ fontSize: 12, color: colors['text-mute'] }}>{label}</Text>
      <Text style={{ fontSize: 13, color: colors.text, fontFamily: PIXEL_BOLD }}>{value}</Text>
    </View>
  );
}
...
function RewardStrip({ rewards }: { rewards: BattleRewards }) {
  ...
  return (
    <View className="mt-s3 rounded-lg border p-s3" style={{ borderColor: `${colors.legendary}45`, backgroundColor: 'rgba(251,191,36,0.06)' }}>
      <Text style={{ fontSize: 9, color: colors.legendary, fontFamily: PIXEL, letterSpacing: 1 }}>REWARDS</Text>
      {nothing ? (
        <Text style={{ marginTop: 3, fontSize: 12, color: colors['text-mute'] }}>No rewards this time — train mode is practice.</Text>
      ) : (
        <View className="mt-s1 flex-row flex-wrap items-center" style={{ gap: 12 }}>
          ...
          {rewards.firstClear ? (
            <Text style={{ fontSize: 10, color: colors.legendary, fontFamily: PIXEL }}>FIRST-CLEAR BONUS</Text>
          ) : null}
        </View>
      )}
    </View>
