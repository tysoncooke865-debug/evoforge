...
- **ERROR MONITORING — WRITTEN, NOT ENABLED (2026-07-25, WO-005, no migration).
  ...
  - **NINTH SESSION (2026-07-26, WO-005 attempt 2 re-run): the install is now a
    MECHANISM, and the eighth session's "no repo file can fix this" was two
    thirds right — the missing third is what had been failing.** Same wall
    confirmed a fourth way (`node -v` → `v24.18.0` runs; `npm --version` via
    Bash and `npm ci --prefix client` via PowerShell both refused, no output),
    so nothing below was executed here either.
    - **Where the eighth session's reasoning was wrong.** It ruled out a
      lifecycle-hook fix on the grounds that only `npm test` runs through npm's
      script runner. But `lint` is an npm script too (`"lint": "expo lint"`),
      so **`prelint` exists**, and a `typecheck` script can be added to earn a
      `pretypecheck`. Three gates, three hooks. `client/scripts/ensure-deps.mjs`
      installs on demand; `package.json` hangs it off all three; an installed
      checkout pays one `existsSync` and prints nothing, so CI (`client.yml`
      already runs `npm ci`) is untouched. **Its correct half stands:** raw
      `npx tsc --noEmit` / `npx expo lint` genuinely cannot be guarded — `npx`
      has no pre-hook — so if the runner's fixed list uses those, the fix it
      needs is still one line of runner config. §5 and `AGENTS.md` now steer
      every reader to the `npm run` forms for that reason.
    - **A SECOND CAUSE OF THE TYPECHECK RED, missed by all eight sessions, that
      `npm ci` alone would never have fixed.** `expo-env.d.ts` is gitignored and
      generated by `expo start` — **not** by `expo export` — so `tsc` on any
      fresh checkout goes red on the `@/global.css` side-effect import even
      after a perfect install. `client.yml` knows this (it writes the one-line
      shim before its Typecheck step, and its comment records it as the first
      CI failure this project ever had); the worktree path never did. The guard
      now writes the same line. Typed routes are on (`typedRoutes: true`), so
      full route-type fidelity still wants `.expo/types/` from an export —
      which §5 and CI both do anyway.
    - **Still owed, unchanged: all of §4 in `docs/ERROR_MONITORING.md`.** No
      monitoring code was touched this session. What changed is that a runner
...
npm run typecheck                            # NOT `npx tsc` — see below
...
**SAYING IT IN FOUR PLACES DID NOT WORK, SO IT IS NOW A MECHANISM**
(2026-07-26, WO-005 attempt 2 re-run). ac736db carried the correct sentence in
all four places — this section, `client/CLAUDE.md`, `docs/ERROR_MONITORING.md`
§4 step 0 and `AGENTS.md` — and the very next run failed with the **identical
three messages**. A doc the runner would have had to read to change its own
behaviour, and didn't, is not a doc gap; it is proof the runner executes a
fixed list. So `client/scripts/ensure-deps.mjs` now does the install itself and
`package.json` hangs it off `pretest` / `prelint` / `pretypecheck`, which npm
runs **whoever typed the command**. An installed checkout pays one `existsSync`
and sees no output; CI's `npm ci` makes it a no-op there.

**Prefer `npm run typecheck` / `npm run lint` over `npx tsc --noEmit` /
`npx expo lint`.** Only the `npm run` forms get the guard: `npx` has no
pre-hook, no config file and no non-interactive auto-install, so a raw `npx`
gate is the one shape that cannot be repaired from inside this repo. If you
own what the runner executes, that is the line to change.

**The typecheck gate had a SECOND cause nobody had found**, and `npm ci` alone
would never have fixed it: `expo-env.d.ts` is gitignored and generated by
`expo start` (not by `expo export`), so `tsc` on any fresh checkout goes red on
the `@/global.css` side-effect import. `client.yml` hit this as its first-ever
CI failure and writes the one-line shim before its Typecheck step;
`ensure-deps.mjs` now writes the same line for the same reason. Full
typed-route fidelity (`.expo/types/`, `typedRoutes: true`) still needs an
export, which both this loop and CI do anyway.
